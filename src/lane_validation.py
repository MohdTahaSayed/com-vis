...
class ValidationConfig:
    ...
    max_width_variation: float = 1.0   # retained for compat, unused
...
class LaneValidation:
    ...
    # ------------------------------------------------------------------
    def _side_ok(self, coeffs: Optional[np.ndarray],
                 y_range: Tuple[int, int], frame_w: int) -> Tuple[bool, str]:
        if coeffs is None:
            return False, "no_fit"
        ...
        sweep = xs.max() - xs.min()
        if sweep > self.cfg.max_sweep_frac * frame_w:
            return False, f"sweep={sweep:.0f}"

        if xs.min() < -0.15 * frame_w or xs.max() > 1.15 * frame_w:
            return False, "off_canvas"

        return True, ""

    # ------------------------------------------------------------------
    def validate(self, left: Optional[np.ndarray],
                 right: Optional[np.ndarray],
                 y_range: Tuple[int, int],
                 frame_w: int) -> ValidationResult:
        l_ok, l_reason = self._side_ok(left, y_range, frame_w)
        r_ok, r_reason = self._side_ok(right, y_range, frame_w)

        pair_ok = False
        pair_reason = "need_both"
        if l_ok and r_ok:
            ys = np.linspace(y_range[0], y_range[1], 50)
            xs_l = left[0] * ys * ys + left[1] * ys + left[2]
            xs_r = right[0] * ys * ys + right[1] * ys + right[2]
            ...
            if np.any(xs_r <= xs_l):
                ...
                if w_min < self.cfg.min_lane_width_px:
                    pair_reason = f"width_min={w_min:.0f}"
                elif w_max > self.cfg.max_lane_width_px:
        ...
        return ValidationResult(
            left_ok=l_ok, right_ok=r_ok, pair_ok=pair_ok,
            reason_left=l_reason, reason_right=r_reason, reason_pair=pair_reason,
        )