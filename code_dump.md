...
Root: C:\Users\xcool\OneDrive\Desktop\video_analytics
Generated: 2026-09-19 20:06:36
Files: 33
...
```yaml
# =====================================================================
# Lane Analytics â€” default configuration
# =====================================================================
...
## File 2 : `docs\writeup.md`

```markdown
# Lane Analytics Pipeline â€” Technical Write-up

**Deliverables:** 4 (lane detection, ego position, lane-change events, signage)
**Dataset:** VBOX0011_Trim.mp4 â€” 720Ã—576, 25 fps, ~1503 s rural Indian highway
...
2. Record the ego vehicle's position within the lane at 1 Hz â†’ CSV
3. Record the time of each lane change â†’ CSV
4. Record roadside signage with location (timestamp) â†’ CSV
...
| Property   | Value                                   |
| ---------- | --------------------------------------- |
| Resolution | 720Ã—576 coded (5:4 DAR ignored)         |
| FPS        | 25 constant                             |
...
* Some stretches have no lane markings â€” MISS is correct
...
```text
Frame (720Ã—576, 25 fps)
â”‚
â”œâ”€â”€â–º [Stage 0] Artifact mask (static bbox, orange reflection)
â”‚
â”œâ”€â”€â–º [Stage 1] LANE DETECTION
â”‚     â€¢ Horizon detection (row-gradient)
â”‚     â€¢ Canny edges (PRIMARY signal)
â”‚     â€¢ Trapezoid ROI mask
â”‚     â€¢ HSV white reinforcement (secondary)
â”‚     â€¢ Hough line segments + slope classification
â”‚     â€¢ Sliding-window 2nd-degree polynomial fit per side
â”‚     â€¢ Geometric validation (sweep, crossing, width)
â”‚     â€¢ Per-side temporal state machine (OK / HOLD / MISS)
â”‚     â€¢ Exponential smoothing of coefficients
â”‚
â”œâ”€â”€â–º [Stage 2] EGO POSITION @ 1 Hz
â”‚     â€¢ Lane centre x at y = 0.85h
â”‚     â€¢ Camera-x = w/2 (vehicle centre reference)
â”‚     â€¢ offset_px = ego_x âˆ’ lane_centre_x
â”‚     â€¢ offset_norm = offset_px / lane_width_px
â”‚     â†’ ego_position.csv
â”‚
â”œâ”€â”€â–º [Stage 3] LANE CHANGE DETECTION
â”‚     â€¢ Rolling-window median of offset_norm
â”‚     â€¢ LEFT / NEUTRAL / RIGHT state classifier
â”‚     â€¢ Event on sign-flip of stable state
â”‚     â†’ lane_changes.csv
â”‚
â””â”€â”€â–º [Stage 4] SIGN DETECTION
      â€¢ YOLOv8n fine-tuned on SDI Indian Traffic Sign dataset
        (6,750 imgs, 85 classes, mAP@50 = 0.92)
      â€¢ IOU tracker for temporal dedup
      â†’ signs.csv
```
...
3. Magnitude and direction:
   ...
   Î¸ = arctan2(Gy, Gx)
   ```
4. Non-maximum suppression along Î¸
5. Hysteresis:
...
* `m < âˆ’0.30` â†’ right lane edge
* `m > +0.30` â†’ left lane edge
* `|m| < 0.30` â†’ discard (near-horizontal = noise)
...
1. Column histogram in bottom 30% of the ROI â†’ peak = window seed
2. 9 horizontal windows stacked bottom-to-top, each:

   ```text
   window_width_frac = 0.10 Ã— w
   ```

   wide
3. Recenter window x to median of collected pixels if count â‰¥ 12
...
* `sweep = max(x) âˆ’ min(x)` over y-range must be:
...
* On valid fit:
  ...
  ```text
  smoothed = (1 âˆ’ Î±) Â· smoothed_prev + Î± Â· current
  ```

* On invalid fit: fail counter increments

* After `hold_frames` â†’ MISS

* After `miss_frames` â†’ full reset
...
lane_width_px = x_right(y*) âˆ’ x_left(y*)

offset_px = x_ego âˆ’ lane_centre_x

offset_norm = offset_px / lane_width_px
```

```text
offset_norm âˆˆ [âˆ’0.5, +0.5]
```
...
```text
med = median(offset_norm[nâˆ’7 : n])

state = LEFT    if med â‰¤ âˆ’0.15
state = RIGHT   if med â‰¥ +0.15
...
**Model â€” YOLOv8n fine-tuned on Indian traffic signs:**

* Backbone: YOLOv8n (COCO pretrained)
* Dataset: SDI "Indian Traffic Sign" â€” 6,750 images, 85 classes
* 50 epochs
* Tesla T4 GPU
...
A classical HSV + shape detector was implemented and evaluated as an alternative (Yadav et al. 2019), but rejected â€” it produced predominantly false positives on blue truck surfaces and commercial billboards, offering no improvement over YOLO. Documented in Â§5.4.

---

## 5. Results

### 5.1 Lane Detection â€” Full Video
...
### 5.2 Ego Position â€” `ego_position.csv`
...
### 5.3 Lane Changes â€” `lane_changes.csv`
...
### 5.4 Sign Detection â€” `signs.csv`
...
The source video is 720Ã—576 coded resolution, captured at 25 fps from a moving vehicle. Roadside signs in the footage occupy approximately 20â€“40 pixels with significant motion blur.

Analysis of individual frames shows YOLOv8n's smallest detection head operates at stride 8. Features smaller than approximately 15 px are unreliable, and 30â€“40 px is the reliable detection floor for the nano variant.
...
on the SDI test set â€” comparable to published state-of-the-art on Indian traffic signs.

On this specific rural-highway footage, the model correctly reported near-zero confidence â€” no false positives were emitted to the CSV.
...
* **Pure HSV-primary lane detection** â€” failed empirically because sky and weathered paint share HSV ranges
* **Hough straight-line only** â€” breaks on curved sections
* **YOLOv8-Seg retraining on Amrita dataset** â€” needs 22k labelled images + GPU days, infeasible in 1 week
* **Bennett SegNet + CamVid** â€” segments road region, not lane geometry; UK domain mismatch
* **Bilal Sinav's pipeline** â€” hardcoded for 1280Ã—720; broken `ym_per_pix`; would need full rewrite

---

## 8. Limitations

1. Daytime only â€” night / rain / wet conditions not validated
2. Sign detection faces source-resolution limitations (see Â§5.4)
3. No metric (meter) offset â€” pixel and normalized-lane offset only, per Prof. Maji's camera-centre guidance
4. MISS stretches during long truck occlusions are correct behaviour

---

## 9. CLI â€” Running on Any Video
...
1. Yadav, S., Patwa, A., Rane, S., Narvekar, C. (2019). *Indian Traffic Signboard Recognition and Driver Alert System Using Machine Learning.* International Journal of Applied Sciences and Smart Technologies, 1(1), 1â€“10.

2. Megalingam, R. K., Thangindala, K., Musani, S. R., Nidamanuru, H., Gadde, L. (2022). *Indian Traffic Sign Detection and Recognition Using Deep Learning.* International Journal of Transportation Science and Technology, 12(4), 683â€“699.
...
5. Canny, J. (1986). *A Computational Approach to Edge Detection.* IEEE Transactions on Pattern Analysis and Machine Intelligence, 8(6), 679â€“698.
...
## File 5 : `scripts\debug_lane_stages.py`
...
## File 6 : `scripts\extract_frames.py`
...
## File 7 : `scripts\inspect_lane_stages.py`
...
## File 8 : `scripts\inspect_roi_horizon.py`
...
## File 9 : `scripts\inspect_sliding_windows.py`
...
## File 11 : `scripts\run_pipeline.py`
...
## File 13 : `scripts\test_stage1_seq.py`
...
class LaneEdges:
    ...
    Public:
        compute(frame, top_y_override=None)
            â†’ (edges_roi, edges_raw, hsv_reinforced)
    """
...
## File 30 : `src\lane_validation.py`
...
## File 31 : `src\sign_categories.py`
